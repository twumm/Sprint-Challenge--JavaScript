...
tyrannosaurus.roar = () => console.log('RAWERSRARARWERSARARARRRR!');
...
const universities = [];
// Loop through the array and get the universities.
for (let i=0; i<graduates.length; i++) {
  universities.push(graduates[i].university);
}
// Sort the new univerties array.
...
const uni = [];
// Loop through sorted universities array.
for (let i=0; i<universities.length; i++) {
  // Check if university name has the string "Uni".
  if (universities[i].indexOf("Uni") >= 0) {
    uni.push(universities[i])
  }
}
console.log('The following universities having the string "Uni":' ,uni);
...
*/
const animalNames = [];
// Loop through zooAnimals array and log both the scientific name and the animal name.
...
// Filter zooAnimals based on animals with population less than 5.
...
zooAnimals.reduce((total, animal) => {
   total = total + animal.population;
   return populationTotal = total
}, 0);
